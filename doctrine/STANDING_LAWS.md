...
## Law 0 — Dual-TF SMA Gate

Longs are allowed only when price is above SMA(4, shift 8, High) **and** SMA(4, shift 8, Low) on **both** M1 and M15.
Shorts are allowed only when price is below both on both timeframes.
If an open trade loses that condition on both TFs → close immediately.

**Current code approximation (do not silently change obs):**
`configs/masks_shell.yaml` forever_masks currently use period 4, shift 4, on 15m/30m/1h.
M1 is not in the forever-mask list and shift differs from Monty’s 8.
Any tightening of the Shell toward the exact Law 0 specification must be done without changing the observation vector that the frozen brains expect. Residual gap is documented here and is a known item for future Shell-only work.
