...
# The knobs the tuner may move. value = base default; bounds keep every proposal sane.
# (rewards/penalties from configs/rewards.yaml; lr/entropy from configs/training.yaml.)
TUNABLE = {
    "w_death_penalty":     (-10.0,  (-40.0, -1.0)),
    "w_did_nothing":       (-6.0,   (-25.0,  0.0)),
    "w_idleness_hunger":   (-0.002, (-0.05,  0.0)),
    "w_day_goal_hit":      (2.0,    (0.0,   12.0)),
    "w_streak_per_day":    (0.15,   (0.0,    3.0)),
    "w_trade_consistency": (0.10,   (0.0,    3.0)),
    "w_net_profit":        (6.0,    (0.5,   25.0)),
    "w_no_drawdown_close": (0.02,   (0.0,    1.0)),
    "lr":                  (3e-4,   (1e-5,  3e-3)),
    "entropy_coef":        (0.01,   (0.0,    0.1)),
}
...
def mutate(config: dict, scale: float, gen: "torch.Generator | None" = None) -> dict:
    ...
    out = dict(config)
    for k, (_, (lo, hi)) in TUNABLE.items():
        step = float(torch.randn(1, generator=gen).item()) * scale
...
def adopt_gate(champ_mask: torch.Tensor, cand_mask: torch.Tensor, z: float = 1.64) -> tuple:
    ...
    numbers). Adopt iff the candidate clears significantly MORE of them than it loses:
      ...
      adopt iff (c - b) >= z * sqrt(b + c)    (one-sided McNemar; self-calibrating margin)
    Returns (adopt: bool, info)."""
    champ = champ_mask.bool(); cand = cand_mask.bool()
    b = int((champ & ~cand).sum().item())
    c = int((~champ & cand).sum().item())
    disagree = b + c
    stat = (c - b) / math.sqrt(disagree) if disagree > 0 else 0.0
    return (stat >= z and c > b), {"b": b, "c": c, "stat": round(stat, 3), "disagree": disagree}
...
if __name__ == "__main__":
    def mask(cleared_idx, n):
        m = torch.zeros(n, dtype=torch.bool); m[cleared_idx] = True; return m

    n = 200
    # champion clears the first 100; candidate clears a clearly-larger, overlapping set
    champ = mask(list(range(100)), n)
    cand_better = mask(list(range(60, 190)), n)     # gains many, loses few -> should ADOPT
    cand_noise = mask(list(range(3, 103)), n)       # ~same count, small shuffle -> should REJECT
    cand_worse = mask(list(range(140, 200)), n)     # clears far fewer -> should REJECT
    ...
    for name, cand, want in [("clearly better", cand_better, True),
        ...
        print("%-16s -> adopt=%-5s want=%-5s  (c=%d gained, b=%d lost, stat=%s)  [%s]"
              % (name, adopt, want, info["c"], info["b"], info["stat"], ok))
    # a mutate sanity check: champion unchanged at scale 0, moves at scale>0, stays in bounds
    g = torch.Generator().manual_seed(0)
    b0 = base_config()
    same = mutate(b0, 0.0, g)
    moved = mutate(b0, 0.3, g)
    in_bounds = all(TUNABLE[k][1][0] <= moved[k] <= TUNABLE[k][1][1] for k in TUNABLE)
