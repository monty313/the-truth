...
CHANGE LOG:
- 2026-07-30: WrongSide — when wrong_side_under_bull high / side_bias_bull low, search up w_with_trend_close and down w_against_trend_close; prove_it gate. — WHY: self_heal_epoch accept
- 2026-07-30  self-heal toolkit: WrongSide class + with/against trend dials (default 0) — WHY: meta searches side-bias cures; MRI must report side_bias/wrong_side.
...
- 2026-07-30: WrongSide — when wrong_side_under_bull high / side_bias_bull low, search up w_with_trend_close and down w_against_trend_close; prove_it gate.

- High `policy_hold` on firm HTF + LTF pull/cont → Policy class → search `w_pullback_with_htf` / `w_setup_skip` (dial); frontier train.
- **WrongSide:** high `wrong_side_under_bull` or low/negative `side_bias_bull` → search ↑ `w_with_trend_close`, ↓ `w_against_trend_close` (defaults 0; meta decides magnitude). Do not freeze a human final weight.
- After TF-set realign, old brains need re-prove + sprint (semantic obs shift).
- Mask veto high → Shell class; do not only chase reward dials.
- Mind Probe must use Categorical.probs; silent pure-hold MRI is invalid.

## Self-heal dials (searchable; default 0 unless noted)

| Dial | Default | When IRAC suggests search |
|------|---------|---------------------------|
| `w_with_trend_close` | 0 | WrongSide / weak side_bias |
| `w_against_trend_close` | 0 | WrongSide (more negative) |
| `w_pullback_with_htf` | 0.25 | Policy hold on pull |
| `w_setup_skip` | 0 | Hold while setup visible |
| `w_quick_pull_close` | 0 | Later: quick with-trend scalps |

Adopt only via prove_it: breach 0%, clear not worse.
