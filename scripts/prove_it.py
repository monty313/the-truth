...
res = evaluate(brain, sim, list(range(D)), n_episodes=D, focus_frac=1.0, decide_every=5, gen=g, ranges=rng)
g2 = torch.Generator(device="cpu").manual_seed(0)
stk = day_after_day_streak(brain, sim, list(range(D)), gen=g2, focus_frac=1.0, decide_every=5, ranges=rng)

di = torch.arange(D); tg = torch.full((D,), TGT); rk = torch.full((D,), RISK)
r = rollout(brain, sim, di, tg, rk, greedy=True, collect=False, decide_every=5)
pnl = r["day_pnl"].float()
