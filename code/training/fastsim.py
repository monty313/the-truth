...
- 2026-07-25  CRITICAL: pull_buy_idx / pull_sell_idx set in __init__ BEFORE
  reload_rewards — WHY: dead code after `return dict(self.w)` caused Colab
  AttributeError: no pull_buy_idx. reload_rewards only updates self.w.
...
class FastSim:
    ...
    def __init__(self, days_obs, days_phys, day_lens, cols, device="cpu",
        ...
        self.w = {k: float(w.get(k, d)) for k, d in {
            ...
            "w_quick_pull_close": 0.08, "quick_pull_max_bars": 20.0,
            "w_setup_skip": -0.02,
    ...
    def reload_rewards(self):
        ...
        Does NOT touch pull_buy_idx / pull_sell_idx / physics — only self.w."""
        w = dict(_load("rewards"))
        defaults = {
            "w_net_profit": 6.0, "w_no_drawdown_close": 0.02, "no_drawdown_tolerance": 0.0,
            "w_pyramid_stack_green": 0.20, "w_pullback_with_htf": 0.25,
