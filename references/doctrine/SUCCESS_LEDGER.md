# SUCCESS LEDGER — What already worked

> **Never erase this file.** It is proof that performance is possible.
> **Flea-jar cure:** the lid is gone. Nothing here is a permanent ceiling.
> When a day *feels* impossible, read this + `doctrine/flea-jar/THE_FLEA_CURE.md`, then run `prove_it`.
> Job: make the possible **consistent** — not debate whether it is possible.

CHANGE LOG:
- 2026-07-25  goal/floor = runtime inputs (meta + self-state) — WHY: change target/risk anytime; measure via prove_it <tgt> <risk>; no full retrain.
- 2026-07-25  agreement agents 80–83 + PART4 — WHY: 70%+ signal evidence; lid off at suggestion layer.
- 2026-07-25  flea-jar banner — WHY: lid is gone; consistency is the only remaining problem.
- 2026-07-25  opened as permanent ledger for fresh start — WHY: start new without forgetting wins.
# NEXT EDITOR: append dated wins only. Do not delete lines.

---

## Standing proofs (do not dilute)

| Proof | What it means |
|--------|----------------|
| **0% breach** on measured PROVEN runs @ 3.0% / 3.5% | Floor can be held while climbing clear rate |
| **~27% clear** (24/90) — `PROVEN_SPRINT_row04_clear24_2026-07-20` | Consistency is trainable, not luck |
| **~21% clear** after TF-set realign | Migration floor — climb from here |
| **90 / 90 days** in swing-capture bound | **Physics does not forbid the goal — the jar lid is off** |
| **policy_hold** on visible pull | Disease is Policy, not “no edge” |
| **w_pullback_with_htf → 0.25** from IRAC | Evidence-backed cure works |
| **Agreement slots 80–83** (see PART4) | Independent families agreeing → **~70–81%** forward hit on signal events |
| **seA ∩ r2A ~75% @ 10 bars** | Strict intersection beats any single family (~60–67%) |
| **goal/floor in obs + meta ranges** | Change target/risk anytime; same brain; prove_it at any pair |

## Brains to warm-start from

| Checkpoint | Note |
|------------|------|
| `PROVEN_SPRINT_row04_clear24_2026-07-20.pt` | Best clear-row lineage |
| `PROVEN_LIFT_2026-07-20.pt` | Lift lineage |
| `PROVEN_2x_2026-07-19.pt` | Earlier 2x |
| `best_trading.pt` | Current best when updated |

## Never delete

- `PERFORMANCE_IS_POSSIBLE.md` (+ PART2 / PART3 / **PART4** / html)
- `doctrine/flea-jar/THE_FLEA_CURE.md`
- `signals/agree.py` + slots 80–83 in `configs/signal_slots.yaml`
- `training/meta_tuner.py`

## Dated wins

- 2026-07-20  PROVEN_SPRINT clear≈27%, row 4, breach 0%
- 2026-07-24  Post SETS-align clear≈21%, breach 0%
- 2026-07-24  IRAC Policy class confirmed (policy_hold on HTF pull)
- 2026-07-25  Fresh start + flea-jar oath locked into CMO doctrine
- 2026-07-25  Agreement agents registered: ~75% / ~70–72% / ~76% / ~78–81% (PART4 tables)
- 2026-07-25  meta_tuner restored on main; goal/floor documented as runtime inputs
- 2026-07-25  Engine restored after d6313e9 gut; signals gated off; prove_it PASS frame_dim=1820 clear=21% breach=0% row=2 avg=+0.17% @ 3.0/3.5
- 2026-07-25  CONSISTENCY_PLAN.md — climb path 21→27→35→50 with breach gate
